...
class AnalystAgent:
    ...
    def _knowledge_alignment_for_service(
        ...
        for hit in hits[:3]:
            ...
            if metric:
                metric_supported = metric in observed_metric_names or metric in supported_dimensions
                if metric_supported:
                    score_delta += 0.04
    ...
    def _build_rule_based_result(self, state: RCAState, prompt: dict[str, Any]) -> dict[str, Any]:
        ...
        for item in metric_results:
            ...
            if item.get("is_anomalous"):
                ...
                if metric_name:
                    service_metric_names.setdefault(service, set()).add(metric_name)

...
from benchmark_case_loader import load_benchmark_case
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--case", default=None, help="Path to a benchmark case directory")
    parser.add_argument(
        "--input",
        default=None,
...
def _resolve_demo_inputs(args: argparse.Namespace) -> tuple[str, str, int | None, int | None, dict[str, Any] | None]:
    if not args.case:
        user_input = args.input or DEFAULT_ANALYSIS_QUESTION
        return args.csv, user_input, args.start, args.end, None

    case = load_benchmark_case(args.case)
    csv_path = case["csv_path"]
    user_input = args.input or case.get("default_user_input") or DEFAULT_ANALYSIS_QUESTION
    start = args.start if args.start is not None else case.get("inject_time")
    end = args.end
    return csv_path, user_input, start, end, case
...
def _print_text_demo(
    ...
    if case:
        ...
        print(f"- case_dir: {case.get('case_dir')}")
        print(f"- inject_time: {case.get('inject_time')}")
...
def main() -> None:
    ...
    csv_path, user_input, start, end, case = _resolve_demo_inputs(args)
